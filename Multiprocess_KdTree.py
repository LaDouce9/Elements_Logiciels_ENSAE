...
def KdTree_2(ann_latitude, ann_longitude, df_dest, plus_proche=1, nbleaf_calcul=None):
    ...
    med_latitude = df_dest['latitude'].median()
    med_longitude = df_dest['longitude'].median()
...
def distance_KDTree_Proc(q,df_orig:  pd.DataFrame,df_dest :pd.DataFrame,plus_proche=1,nbleaf_calcul=None):
...
def Distance_MultiProcess_KDTree(df_origin: pd.DataFrame, df_destination: pd.DataFrame, nProc=2,plus_proche=1,nbleaf_calcul=None):
    """
    ...
    for p in range(nProc):
        ...
        x = ctx.Process(target=distance_KDTree_Proc,
                        args=(q, df_origin.iloc[index_a[p]:index_b[p], :], df_destination,\
                              plus_proche,nbleaf_calcul))
        process.append(x)
...
if __name__ == '__main__':
    ...
    for nb_proc in nb_process:
        print("MultiprocessKDT starting for {} Process".format(int(nb_proc)))
        t = time.time()
        nb_dest_point_distance_normal = 100 #nombre de points à partir duquel on repasse en calcul de distance  classique
        res = Distance_MultiProcess_KDTree(df_origin2, df_dest2, nProc=nb_proc,plus_proche=1 ,\
                                           nbleaf_calcul=nb_dest_point_distance_normal)
        # Calcul distance la plus proche  pour  chaque annonce  puis  export
        pd.DataFrame(res[0]).to_excel('Res_Process_KDT/MultiProcess_N_' + str(nb_proc) + '.xlsx')
    ...
    for i in range(len(time_process)):
        ...
        plt.ylabel('time (s)')
    fig.tight_layout()
    plt.savefig('KDT_time_per_process.jpg', dpi=300)
    plt.show()
    ...
    plt.savefig('KDT_time_process.jpg', dpi=300)
    plt.show()
    ...
    plt.legend(loc='best')
    plt.savefig('KDT_time_process_com.jpg', dpi=300)
    plt.show()
